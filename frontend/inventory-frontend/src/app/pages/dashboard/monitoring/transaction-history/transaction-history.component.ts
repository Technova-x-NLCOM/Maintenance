...
export class TransactionHistoryComponent implements OnInit {
  ...
  load(page = 1): void {
    ...
    this.http.get<Paginated<TransactionRecord>>(this.BASE, { headers: this.authHeaders(), params: p }).subscribe({
      next: (res) => { this.transactions = res.data.data; this.page = res.data.current_page; this.lastPage = res.data.last_page; this.total = res.data.total; this.loading = false; this.cdr.detectChanges(); },
      error: () => { this.error = 'Failed to load transactions.'; this.loading = false; this.cdr.detectChanges(); }
    });
  }

  apply(): void { this.load(1); }
  clear(): void { this.search = ''; this.type = ''; this.dateFrom = ''; this.dateTo = ''; this.load(1); }
  ...
  exportExcel(): void {
    ...
    const dataRows: any[][] = [
      ...
      ...this.transactions.map(r => [r.transaction_type, r.reference_number, r.item_code, r.item_description, r.batch_number ?? '—', r.quantity, r.measurement_unit ?? '—', r.destination ?? r.reason ?? '—', r.performed_by_name, this.datePipe.transform(r.transaction_date, 'MMM d, y, h:mm a') ?? r.transaction_date])
    ];
    const wsData = [['NLCOM - IMS'], ['Transaction History'], [`Generated: ${dateStr}`], [], ...dataRows];
  ...
  exportPdf(): void {
    ...
    doc.setFontSize(11); doc.setTextColor(51, 65, 85); doc.text('Transaction History', 40, 54);
    ...
    autoTable(doc, {
      ...
      body: this.transactions.map(r => [r.transaction_type, r.reference_number, r.item_code, r.item_description, r.batch_number ?? '—', r.quantity, r.measurement_unit ?? '—', r.destination ?? r.reason ?? '—', r.performed_by_name, this.datePipe.transform(r.transaction_date, 'MMM d, y, h:mm a') ?? r.transaction_date]),
